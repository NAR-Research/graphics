...
function sendHeightToParent() {
  ...
  const height = Math.max(
    root ? Math.ceil(root.scrollHeight) : 0,
    Math.ceil(body.scrollHeight),
    Math.ceil(body.offsetHeight),
    Math.ceil(html.clientHeight),
    Math.ceil(html.scrollHeight),
    Math.ceil(html.offsetHeight)
  );

  window.parent.postMessage(
    {
      type: "nar-dashboard-height",
      height
    },
    "*"
  );
...
function renderAllCharts() {
  ...
  setTimeout(scheduleHeightUpdate, 900);
}
...
function initAutoHeightObservers() {
  ...
  const logo = document.querySelector(".source-logo");
  if (logo) {
    logo.addEventListener("load", scheduleHeightUpdate);
  }
}

renderAllCharts();
initToggle();
initAutoHeightObservers();